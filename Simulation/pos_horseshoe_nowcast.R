...
###########################Theta#########
####no cons2
theta_update_lag<- function(data,factor,Omega,mu_x,rel){
  #tau scalar
  #lambda n*1 vector
  ...
  if (rel!=3){t=t_null+1}else{t=t_null}
  ...
  if (rel==1){
    ...
    W=tmp1+t(Ct1)%*%t(I1)%*%solve(omg_t1)%*%I1%*%Ct1+diag(n*r)
    inv_W<-solve(W,tol=1e-20)
    U=inv_W%*%(tmp2+t(Ct1)%*%t(I1)%*%solve(omg_t1)%*%I1%*%(x[,t-1]-mu))
  }else if (rel==2){
    omg_t2<-I2%*%omg%*%t(I2)
    Ct2<-kronecker(diag(n),t(FT[,t-1]))
    
    W=tmp1+t(Ct2)%*%t(I2)%*%solve(omg_t2)%*%I2%*%Ct2+diag(n*r)
    inv_W<-solve(W,tol=1e-20)
    U=inv_W%*%(tmp2+t(Ct2)%*%t(I2)%*%solve(omg_t2)%*%I2%*%(x[,t-1]-mu))
  }else{ ##rel==3
    ...
    omg_t1<-I1%*%omg%*%t(I1)
    Ct1<-kronecker(diag(n),t(FT[,t]))
    
    W=tmp1+t(Ct2)%*%t(I2)%*%solve(omg_t2)%*%I2%*%Ct2+
      t(Ct1)%*%t(I1)%*%solve(omg_t1)%*%I1%*%Ct1+diag(n*r)
    inv_W<-solve(W,tol=1e-20)
    
    U=inv_W%*%(tmp2+t(Ct2)%*%t(I2)%*%solve(omg_t2)%*%I2%*%(x[,t-1]-mu)+
                 t(Ct1)%*%t(I1)%*%solve(omg_t1)%*%I1%*%(x[,t]-mu))
  }
...
omega_update_lag<- function(data,factor,Theta,rel,mu_x,Omega){
  #mu_x be n*1 vector
  x=as.matrix(data)
  n=dim(x)[1]
  t_null=dim(x)[2]
  ...
  if (rel==1){
   CT=kronecker(diag(rep(1,n)),t(FT[,t-1]))
   H<-I1%*%(x[,t-1]-mu_x-CT%*%theta)%*%t((x[,t-1]-mu_x-CT%*%theta))%*%t(I1)
   
   p1<-(det(I1%*%o_new%*%t(I1))/det(I1%*%o_old%*%t(I1)))^-0.5
   p2<-exp(-0.5*sum(diag(H%*%(solve(I1%*%o_new%*%t(I1))-solve(I1%*%o_old%*%t(I1))))))
  }else if (rel==2){
    CT=kronecker(diag(rep(1,n)),t(FT[,t-1]))
    H<-I2%*%(x[,t-1]-mu_x-CT%*%theta)%*%t((x[,t-1]-mu_x-CT%*%theta))%*%t(I2)
    
    p1<-(det(I2%*%o_new%*%t(I2))/det(I2%*%o_old%*%t(I2)))^-0.5
    p2<-exp(-0.5*sum(diag(H%*%(solve(I2%*%o_new%*%t(I2))-solve(I2%*%o_old%*%t(I2))))))
  }else{ #rel==3
    CT1=kronecker(diag(rep(1,n)),t(FT[,t-1]))
    CT=kronecker(diag(rep(1,n)),t(FT[,t]))
    
    H1<-I2%*%(x[,t-1]-mu_x-CT1%*%theta)%*%t((x[,t-1]-mu_x-CT1%*%theta))%*%t(I2)
    HT<-I1%*%(x[,t]-mu_x-CT%*%theta)%*%t((x[,t]-mu_x-CT%*%theta))%*%t(I1)
    
    p1<-((det(I1%*%o_new%*%t(I1))*det(I2%*%o_new%*%t(I2)))/
           (det(I1%*%o_old%*%t(I1))*det(I2%*%o_old%*%t(I2))))^-0.5
    
    p2<-exp(-0.5*sum(diag(H1%*%(solve(I2%*%o_new%*%t(I2))-solve(I2%*%o_old%*%t(I2)))))-
              0.5*sum(diag(HT%*%(solve(I1%*%o_new%*%t(I1))-solve(I1%*%o_old%*%t(I1))))))
  }
...
ft_update_lag<- function(data,GDP,mu_x,factor,Theta,A,Omega,Sigma,eta2,rel,
  ...
  midF=matrix(0,nrow=r,ncol=3*k-2-3)
  for (i in 4:(3*k-2)){
    if (i%%3==1){
  ...
  midF2=matrix(0,nrow=r,ncol=(t-3*k))
  for (i in (3*k-1):(t-2)){
  ...
  if (rel==1){
    ...
    res=cbind(startF1,startF,midF,midF2,Ft1)
  }else if (rel==2){
    
    nj1<-dim(I2)[1]
    tilde_Y_t1=c(I2%*%(x[,t-1]-mu_x),tmpF)
    tilde_X_t1=rbind(I2%*%Theta,tilde_A)
    Omega_T1<-I2%*%Omega%*%t(I2)
    s_tilde_Sigma_t1=cbind(rbind(solve(Omega_T1),matrix(0,nrow=p,ncol=nj1)),
                           rbind(matrix(0,nrow=nj1,ncol=p),s_tmp))
    Wt1=t(tilde_X_t1)%*%s_tilde_Sigma_t1%*%tilde_X_t1
    Mt1=t(tilde_X_t1)%*%s_tilde_Sigma_t1%*%tilde_Y_t1
    ...
    nj<-dim(I1)[1]
    tilde_Y_t=c(I1%*%(x[,t]-mu_x),tmpF)
    tilde_X_t=rbind(I1%*%Theta,tilde_A)
    Omega_T<-I1%*%Omega%*%t(I1)
...
######mu########
mu_update_lag<-function(omega,rel,data,factor,theta){
  The<-as.matrix(theta)
  FT<-as.matrix(factor)
  Omg<-as.matrix(omega)
  X<-as.matrix(data)
  ...
  if (rel==1){
   inv_sig<-(t-2)*solve(Omg)+diag(n)+t(I1)%*%solve(I1%*%Omg%*%t(I1))%*%I1
   
   mean_mu<-solve(inv_sig)%*%(tmp+t(I1)%*%solve(I1%*%Omg%*%t(I1))%*%I1%*%
                                (X[,t-1]-The%*%FT[,t-1]))
  }else if (rel==2){
   inv_sig<-(t-2)*solve(Omg)+diag(n)+t(I2)%*%solve(I2%*%Omg%*%t(I2))%*%I2
